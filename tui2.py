...
def run_train_time(bdfs):
    bdf_str = ",".join(bdfs)
    subprocess.run(["python3", "train_time.py"], input=bdf_str.encode(), check=True)

def get_adjusted_time():
    try:
        with open("adjusted_time.txt", "r") as f:
            adjusted_time = float(f.read().strip())
...
def main(stdscr):
    ...
    bdfs = [gpu_info_list[slot][0] for slot in slotlist]

    # Run train_time to determine adjusted time
    run_train_time(bdfs)
    ...
    input_window.addstr(10-2, 0, f"Adjusted Sleep Time: {adjusted_time:.2f} seconds")
    ...
    sbr.run_test(stdscr, user_password, inputnum_loops, kill, slotlist, adjusted_time)
