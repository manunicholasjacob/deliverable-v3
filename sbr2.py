...
# Read the adjusted time from the file
def read_adjusted_time():
    try:
        with open("adjusted_time.txt", "r") as f:
            adjusted_time = float(f.read().strip())
        return adjusted_time
    except FileNotFoundError:
        print("adjusted_time.txt not found. Using default value.")
        return 0.3
    except ValueError:
...
def run_test(stdscr, user_password, inputnum_loops, kill, slotlist, adjusted_time):
    ...
    output_lines.append(f"Slot Test Counts: {num_loops}")
